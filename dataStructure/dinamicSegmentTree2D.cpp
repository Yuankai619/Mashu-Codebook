...
struct segy {
    ...
    Node* root;
    void init(int _n=1e9) {n = _n; root = new Node();}
    void pull(Node* node) {node->val = (node->l?node->l->val:0) + (node->r?node->r->val:0);}
    ...
    void add(int pos, int val) {add(root, 0, n, pos, val);}
};
struct segx {
    struct Node {
        segy * tree_y;
    ...
    int query(Node* cur, int lx, int rx, int qlx, int qrx, int qly, int qry) {
        if(!cur) return 0;
        if(qlx<=lx && rx<=qrx) {return cur->tree_y->query(qly, qry);}
        int mid = lx+rx>>1;
        int ans = 0;
    ...
    int query(int qlx, int qly, int qrx, int qry) {return query(root, 0, n, qlx, qrx, qly, qry);}
    void add(int qx, int qy, int val) {add(root, 0, n, qx, qy, val);}
} tree;